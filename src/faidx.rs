...
extern "C" {
    ...
    fn fai_path(fa: *const c_char) -> *mut c_char;
}
...
impl Faidx {
    ...
    pub fn fetch_seq<S: AsRef<str>>(&self, cname: S) -> io::Result<Sequence> {
        ...
        if let Some(slen) = self.seq_len(cname) {
            if slen == 0 {
                Err(hts_err(format!("Sequence {} has zero length", cname)))
            } else {
                let mut len: HtsPos = 0;
                let seq = unsafe {
                    faidx_fetch_seq64(
                        self.inner(),
                        get_cstr(cname).as_ptr(),
                        0,
                        (slen - 1) as HtsPos,
                        &mut len,
                    )
                ...
                if len == -2 {
                    Err(hts_err(format!("Sequence {} not found", cname)))
                } else if len < 0 || seq.is_null() {
                    Err(hts_err(format!(
                        "Loading of sequence data for {} failed",
                        cname
                    )))
                } else {
                    Ok(Sequence {
...
pub fn faidx_load<S: AsRef<str>>(name: S) -> io::Result<Faidx> {
    ...
    match NonNull::new(unsafe { fai_load(get_cstr(name).as_ptr()) }) {
        None => Err(hts_err(format!(
            "Failed to load reference file index {}",
            name
        ))),
...
impl Sequence {
    pub fn get_seq(&self, x: usize, y: usize) -> io::Result<&[u8]> {
        if x > y {
            Err(hts_err("Invalid coordinates (start > end)".to_string()))
        } else {
...
impl FaiPath {
    pub fn new<S: AsRef<str>>(name: S) -> Option<Self> {
        let name = name.as_ref();
        NonNull::new(unsafe { fai_path(get_cstr(name).as_ptr()) }).map(|p| FaiPath { inner: p })
    }

    pub fn cpath(&self) -> *const c_char {
        unsafe { self.inner.as_ref() }
    }

    pub fn path(&self) -> &str {
        from_cstr(self.cpath())
    }
}
