...
use super::{
    hts_err, hts_idx_t, hts_itr_next, kstring_t, Hts, HtsFileDesc, HtsItr, HtsPos, HtsRead, BGZF,
};
...
pub const tbx_conf_vcf: tbx_conf_t = tbx_conf_t {
    preset: TBX_VCF,
    sc: 1,
    bc: 2,
    ec: 0,
...
impl TbxRec {
    ...
    pub fn parse(&mut self, tbx: &mut tbx_t) -> io::Result<()> {
        ...
        if let Some(p) = self.line.as_slice_mut(true) {
            ...
            let get_num = |p: &[u8]| -> io::Result<HtsPos> {
                unsafe {
                        ...
                        match preset & 0xffff {
                            ...
                            TBX_SAM if id == 6 => {
                                ...
                                for c in p_curr.iter() {
                                    match *c {
                                        x if x >= b'0' && x <= b'9' => {
                                            op_len = (op_len * 10) + ((x - b'0') as HtsPos)
                                        }
                                        b'M' | b'D' | b'N' => {
                                            len += op_len;
                                            op_len = 0;
            ...
            Err(hts_err(
                "Can not parse tbx record from empty string".to_string(),
            ))
        }
    }
    ...
    pub fn skip_line(&mut self, hts: &mut Hts) -> io::Result<bool> {
        let (fp, tbx) = hts.hts_file_and_tbx();
        if let Some(HtsFileDesc::Bgzf(bgzf)) = fp.file_desc() {
            let res = if let Some(tbx) = tbx {
                match unsafe { bgzf_getline(bgzf.as_ptr(), 10, &mut self.line as *mut kstring_t) } {
                    0..=c_int::MAX => Ok(true),
                    -1 => Ok(false),
                ...
                Err(hts_err(format!(
                    "Appropriate header not found for file {}",
                    fp.name()
                )))
            };
            res
            ...
            Err(hts_err(format!(
                "File {} is not in bgzf format (required for tabix)",
                fp.name()
            )))
        }
    }
...
impl DerefMut for Tbx {
    #[inline]
    fn deref_mut(&mut self) -> &mut tbx_t {
...
impl Tbx {
    pub fn new(inner: NonNull<tbx_t>) -> Self {
        Self {
            inner,
            phantom: PhantomData,
        }
    ...
    pub fn load<S: AsRef<Path>>(name: S) -> io::Result<Self> {
        ...
        match NonNull::new(unsafe { tbx_index_load3(cname.as_ptr(), null::<c_char>(), 0) }) {
            None => Err(hts_err(format!(
                "Couldn't open tabix index for file {}",
                name.display()
            ))),
            Some(p) => Ok(Tbx {
                inner: p,
...
pub(crate) unsafe extern "C" fn tbx_read_itr(
    fp: *mut BGZF,
    data: *mut c_void,
    r: *mut c_void,
    tid: *mut c_int,
    ...
    match bgzf_getline(fp, b'\n' as c_int, &mut tbx_rec.line) {
        c if c >= 0 => match tbx_rec.parse(tbx) {
            Ok(_) => {
                *tid = tbx_rec.tid;
                *beg = tbx_rec.begin;
                *end = tbx_rec.end;
